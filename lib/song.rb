...
# Build a class finder Song.find_by_name that accepts the string name of a song and returns the matching instance of the song with that name
  ...
  def self.find_by_name(string_name_of_the_song)
    self.all.detect {|i| i.name == string_name_of_the_song} #all?(pattern) true/false
  end
  
  #Song.find_or_create_by_name class method. This method will accept a string name for a song and either return a matching song instance with that name or create a new song with the name and return the song instance

  def self.find_or_create_by_name(string_name_of_the_song)
    #found_it = self.all.detect {|i| i.name == string_name_of_the_song}
    found_it = find_by_name(string_name_of_the_song)
    if found_it == nil
      # song = self.new
      # song.name = string_name_of_the_song
      # song.save
      # song
      create_by_name(string_name_of_the_song)
    else
      found_it
    end
end
